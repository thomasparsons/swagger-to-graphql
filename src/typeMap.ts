...
var isEnumType = function isEnumType(jsonSchema) {
...
var jsonSchemaTypeToGraphQL = function jsonSchemaTypeToGraphQL(title, jsonSchema, schemaName, isInputType, gqlTypes) {
  var baseType = function () {
    ...
    if (isEnumType(jsonSchema)) {
      return createGQLEnum(jsonSchema, title + '_' + schemaName, isInputType, gqlTypes);
    }
...
var createGQLEnum = exports.createGQLEnum = function createGQLEnum(jsonSchema, title, isInputType, gqlTypes) {
  title = jsonSchema && jsonSchema.title || title || '';
  title = makeValidName(title);
  ...
  jsonSchema.enum.map(e => enumValues[e] = { value: e})
  ...
  if (isInputType) {
    ...
    result = new graphql.GraphQLEnumType({
      name: title,
      description: description,
      values: enumValues
    });
